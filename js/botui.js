...
    let type = "type";
    ...
    var other = function() {
        botui.message.bot({
            ...
            return botui.message.bot({
                delay: 1500,
                content: "这是个悠长的故事...还是...以后有机会慢慢道来，喝杯茶吧！"
            })
        }).then(function(){
              work(); //主调函数
        })
...
            }
        };
        xmlhttp.open("GET", "https://v2.alapi.cn/api/joke/random?token=NK8MqnIy5hqGsxMX");
        xmlhttp.send();
    }
    // end
    // riddleapi
...
            }
        };
        xmlhttp.open("GET", "https://v2.alapi.cn/api/riddle/random?token=NK8MqnIy5hqGsxMX");
        xmlhttp.send();
    }
    // end
    // commonsenseapi
    var getcsApi = function(){
                var xmlhttp = new XMLHttpRequest();
                xmlhttp.onreadystatechange = function() {
                    if (this.readyState == 4 && this.status == 200) {
                    myObj = JSON.parse(this.responseText);
                    cstitle = myObj.newslist[0].title;
                    csanswer = myObj.newslist[0].answer;
                    csanalyse = myObj.newslist[0].analyse;
                    }
                };
                xmlhttp.open("GET", "https://api.tianapi.com/decide/index?key=b6a0eb2f344639bb9e0d36d589565612");
                xmlhttp.send();
            }
    //end
    var commonsense = function(){
        botui.message.bot({
            delay: 3000,
            content: "常识小测试开始啦"
        }).then(function(){
...
                    value: "correct"
            ...
            if(res.value=="correct" && csanswer=="1"){
                return botui.message.bot({
                    delay: 1500,
                    content: "恭喜你答对啦！看看你与本机器人的分析想得一样吗🎀"
                }).then(function(){
                    return botui.message.bot({
                        delay: 1500,
                        content: csanalyse
            ...
            else if(res.value=="incorrect" && csanswer=="0"){
                return botui.message.bot({
                    delay: 1500,
                    content: "你答对啦！看看你与本机器人的分析想得一样吗🎀"
                }).then(function(){
                    return botui.message.bot({
                        delay: 1500,
                        content: csanalyse
